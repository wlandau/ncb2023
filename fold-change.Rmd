...
Likewise, a 90% posterior interval can be constructed from the 5th and 95th percentiles of the reciprocals $1 / \theta^{(1)}, \ldots, 1 / \theta^{(M)}$ of the original posterior samples. Unlike the frequentist case, this process does not require extra approximations like the Delta method, and it does not require Jacobian adjustments, no matter how complicated the transformation of variables may be. In the next section, we walk through an example of fitting a Bayesian model with MCMC, and we emphasize the simplicity and convenience of Bayesian inference on transformed parameters.
